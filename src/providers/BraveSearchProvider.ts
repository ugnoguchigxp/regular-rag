...
export class BraveSearchProvider implements WebSearchProvider {
    ...
    constructor(apiKey: string) {
        if (!apiKey) {
            throw new Error("BRAVE_SEARCH_API_KEY is required for BraveSearchProvider");
        }
    ...
    async search(options: WebSearchOptions): Promise<WebSearchResult[]> {
        ...
        try {
            const data = await fetchJson<any>(`${this.baseURL}/web/search`, {
                params,
            ...
            return data.web.results.map((item: any, idx: number) => ({
                title: item.title || "",
                url: item.url || "",
